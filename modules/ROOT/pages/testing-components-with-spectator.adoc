...
We have already used small <<testing-components#_testing_helpers, element testing helpers>>. They solve isolated problems in order to write more consistent and compact specs.
...
TIP: Unified testing API

*https://github.com/ngneat/spectator[Spectator]* is an opinionated library for testing Angular applications. Technically, it sits on top of `TestBed`, `ComponentFixture` and `DebugElement`. But the main idea is to unify all these APIs in one consistent, powerful and user-friendly interface – the `Spectator` object.
...
For <<testing-components-with-children#_faking_a_child_component_with_ng_mocks, faking child Components>>, Spectator resorts to the ng-mocks library just like we did.

This guide cannot introduce all Spectator features, but we will discuss the basics of Component testing using Spectator.

Both <<example-applications#_example_applications, example applications>> are tested with our element helpers and also with Spectator. The former specs use the suffix `.spec.ts`, while the latter use the suffix `.spectator.spec.ts`. This way, you can compare the tests side-by-side.

In this chapter, we will discuss testing the Flickr search with Spectator.

== Component with an Input

Let us start with the https://github.com/9elements/angular-flickr-search/tree/main/src/app/components/full-photo[`FullPhotoComponent`] because it is a <<testing-components-with-children#_testing_components_with_children, presentational Component>>, a leaf in the Component tree. It expects a `Photo` object as Input and renders an image as well as the photo metadata. No Outputs, no children, no Service dependencies.

The https://github.com/9elements/angular-flickr-search/blob/main/src/app/components/full-photo/full-photo.component.spec.ts[`FullPhotoComponent` suite with our helpers] looks like this:
...
`createComponentFactory` expects a configuration object. `component: FullPhotoComponent` specifies the Component under test. `shallow: true` means we want <<testing-components-with-children#_shallow_vs_deep_rendering, shallow, not deep rendering>>. It does not make a difference for `FullPhotoComponent` though since it has no children.
...
The central `spectator.query` method finds an element in the DOM. This guide recommends to <<testing-components#_querying_the_dom_with_test_ids, find elements by test ids>> (`data-testid` attributes).
...
Compared to the version with custom testing helpers, the Spectator version is not necessarily shorter. But it works on a _consistent abstraction level_.
...
[NOTE]
====
- https://github.com/9elements/angular-flickr-search/tree/main/src/app/components/full-photo[FullPhotoComponent: implementation code and the two tests]
- https://github.com/ngneat/spectator#queries[Spectator: Queries]
- https://github.com/ngneat/spectator#custom-matchers[Spectator: Custom matchers]
====

== Component with children and Service dependency

Spectator really shines when testing <<testing-components-with-children#_testing_components_with_children, container Components>>. These are Components with children and Service dependencies.

In the Flickr search, the topmost https://github.com/9elements/angular-flickr-search/tree/main/src/app/components/flickr-search[`FlickrSearchComponent`] calls the `FlickrService` and holds the state. It orchestrates three other Components, passes down the state and listens for Outputs.
...
TIP: Without Spectator

The https://github.com/9elements/angular-flickr-search/blob/main/src/app/components/flickr-search/flickr-search.component.spec.ts[`FlickrSearchComponent` test suite with our helpers] looks like this:
...
- We use <<testing-components-with-children#_shallow_vs_deep_rendering, shallow rendering>>. The child Components are not declared and only empty shell elements are rendered (`app-search-form`, `app-photo-list` and `app-full-photo`). This lets us check their presence, their Inputs and Outputs.
...
1. We replace the child Components with fakes created by <<testing-components-with-children#_faking_a_child_component_with_ng_mocks, ng-mocks>>. The fake Components mimic the originals regarding their Inputs and Outputs, but they do not render anything. We will work with these Component instances instead of operating on `DebugElement`s.
...
Due to the <<faking-dependencies#_equivalence_of_fake_and_original, equivalence of fake and original>>, we can access Inputs with the pattern `componentInstance.input`. And we let an Output emit with the pattern `componentInstance.output.emit(…)`.
...
TIP: Manual type guard

Unfortunately, `expect` is not a https://www.typescriptlang.org/docs/handbook/advanced-types.html[TypeScript type guard]. Jasmine expectations cannot narrow down the type from `PhotoListComponent | null` to `PhotoListComponent`.
...
[NOTE]
====
- https://github.com/9elements/angular-flickr-search/tree/main/src/app/components/flickr-search[FlickrSearchComponent: implementation code and the two tests]
- https://github.com/help-me-mom/ng-mocks#how-to-create-a-mock-component[ng-mocks: How to mock a component]
- https://github.com/ngneat/spectator#mocking-providers[Spectator: Mocking providers]
====
...
To perform a simple click, we use `spectator.click` and pass the target element or a `byTestId` selector. An example from the https://github.com/9elements/angular-flickr-search/blob/main/src/app/components/photo-item/photo-item.component.spectator.spec.ts[PhotoItemComponent test]:
...
Another common task is to simulate form field input. So far, we have used the <<testing-components#_filling_out_forms, `setFieldValue` helper>> for this purpose.

TIP: <p><code>spectator.&#x200b;typeInElement</code></p>

Spectator has an equivalent method named `spectator.typeInElement`. It is used by the https://github.com/9elements/angular-flickr-search/blob/main/src/app/components/search-form/search-form.component.spectator.spec.ts[SearchFormComponent test]:
...
[NOTE]
====
- https://github.com/9elements/angular-flickr-search/tree/main/src/app/components/photo-item[PhotoItemComponent: implementation code and the two tests]
- https://github.com/9elements/angular-flickr-search/tree/main/src/app/components/search-form[SearchFormComponent: implementation code and the two tests]
- https://github.com/ngneat/spectator#events-api[Spectator: Events API]
====
...
[NOTE]
====
- https://github.com/ngneat/spectator[Spectator project site]
- https://github.com/help-me-mom/ng-mocks[ng-mocks project site]
====

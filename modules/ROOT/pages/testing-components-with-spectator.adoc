...
We have already used small ../testing-components/#testing-helpers[element testing helpers]. They solve isolated problems in order to write more consistent and compact specs.
...
For ../testing-components-with-children/#faking-a-child-component-with-ng-mocks[faking child Components], Spectator resorts to the ng-mocks library just like we did.
...
`createComponentFactory` expects a configuration object. `component: FullPhotoComponent` specifies the Component under test. `shallow: true` means we want ../testing-components-with-children/#shallow-vs-deep-rendering[shallow, not deep rendering]. It does not make a difference for `FullPhotoComponent` though since it has no children.
...
The spec `it('renders the photo information', /_ … _/)` repeats three essential tasks several times:
...
The central `spectator.query` method finds an element in the DOM. This guide recommends to ../testing-components/#querying-the-dom-with-test-ids[find elements by test ids] (`data-testid` attributes).
...
Spectator really shines when testing ../testing-components-with-children/#testing-components-with-children[container Components]. These are Components with children and Service dependencies.
...
- We use ../testing-components-with-children/#shallow-vs-deep-rendering[shallow rendering]. The child Components are not declared and only empty shell elements are rendered (`app-search-form`, `app-photo-list` and `app-full-photo`). This lets us check their presence, their Inputs and Outputs.
...
1. We replace the child Components with fakes created by ../testing-components-with-children/#faking-a-child-component-with-ng-mocks[ng-mocks]. The fake Components mimic the originals regarding their Inputs and Outputs, but they do not render anything. We will work with these Component instances instead of operating on `DebugElement`s.
...
describe('FlickrSearchComponent with spectator', () => {
  ...
  const createComponent = createComponentFactory(/_ … _/);
...
Due to the ../faking-dependencies/#equivalence-of-fake-and-original[equivalence of fake and original], we can access Inputs with the pattern `componentInstance.input`. And we let an Output emit with the pattern `componentInstance.output.emit(…)`.
...
Another common task is to simulate form field input. So far, we have used the ../testing-components/#filling-out-forms[`setFieldValue` helper] for this purpose.

...
We have already learned how to ../testing-components/#filling-out-forms[fill out form fields] when testing the counter Component. In doing so, we developed the `setFieldValue` testing helper.
...
The `SignupFormComponent` depends on the `SignupService`. We do not want HTTP requests to the back-end when the tests run, so we ../testing-components-depending-on-services/#faking-service-dependencies[replace the Service with a fake instance].
...
This is a job for Jasmine’s `createSpyObj` (see ../testing-components-depending-on-services/#faking-service-dependencies[Faking Service dependencies]).
...
The `fillForm` function lies in the scope of `describe` so it may access the `fixture` variable. It uses the `setFieldValue` and `checkField` ../testing-components/#testing-helpers[element testing helpers].
...
We find the `form` element by its test id and simulate a `submit` event (see ../testing-components/#triggering-event-handlers[Triggering event handlers]).
...
In consequence, the test needs to wait one second for the asynchronous validators. An easy way would be to write an asynchronous test that uses `setTimeout(() => { /_ … _/}, 1000)`. But this would slow down our specs.
...
As stated above, the two other specs `it('fails if the email is taken', /_ … _/)` and `it('fails if the password is too weak', /_ … _/)` look the same apart from the fake setup.

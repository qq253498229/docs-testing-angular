...
When testing a piece of code, you need to decide between an <<testing-principles#_integration_tests, integration test>> and a <<testing-principles#_unit_tests, unit test>>. To recap, the integration test includes (“integrates”) the dependencies. In contrast, the unit test replaces the dependencies with fakes in order to isolate the code under test.
...
Again, this is a plain TypeScript example to illustrate the usage of spies. Usually, an Angular Service does not use `fetch` directly but uses `HttpClient` instead. We will get to know testing this later (see <<testing-services#_testing_a_service_that_sends_http_requests, Testing a Service that sends HTTP requests>>).

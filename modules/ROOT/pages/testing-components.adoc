...
`createComponent` 将组件渲染到 HTML DOM 中的一个 `div` 容器元素中。然而，还有一些东西缺失。组件没有完全渲染出来。所有静态的 HTML 存在，但动态的 HTML 却缺失了。模板绑定，如示例中的 `{% raw %}{{ count }}{% endraw %}`，没有被计算。
...
我们已经提到在测试环境中，Angular不会自动检测变更以更新DOM。点击增加按钮会更改组件实例的 `count` 属性。要更新模板绑定 `{% raw %}{{ count }}{% endraw %}`，我们需要 _手动触发变更检测_。

...
[NOTE]
====
- <<test-suites-with-jasmine#_test_suites_with_jasmine, Test suites with Jasmine>>
====
...
The Angular testing tools are neutral when it comes to DOM querying. They tolerate different approaches. After consideration, you should opt for one specific solution, document it as a <<angular-testing-principles#_testing_conventions, testing convention>> and apply it consistently across all tests.
...
Your testing helpers should cast your <<angular-testing-principles#+testing_conventions, testing conventions>> into code. They not only improve the individual test, but make sure all tests use the same patterns and work the same.
...
While the reset feature is simple, this is how to test most form logic. Later, we will learn how to <<testing-complex-forms#_testing_complex_forms, test complex forms>>.
...
Per default, Jasmine warns you that the spec has no expectations but treats the spec as successful (see <<angular-testing-principles#_configuring_karma_and_jasmine, Configuring Karma and Jasmine>>). We want the spec to fail explicitly in this case, so we make sure the expectation is always run.
...
These tests are black box tests. We have already talked about <<testing-principles#_black_box_vs_white_box_testing, black box vs. white box testing>> in theory. Both are valid testing methods. As stated, this guide advises to use black box testing first and foremost.

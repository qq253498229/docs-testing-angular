...
How do we verify that `console.log` has been called? We can ../faking-dependencies/#spying-on-existing-methods[spy on existing methods] with Jasmine’s `spyOn`.
...
Remember the ../faking-dependencies/#faking-dependencies[rules for faking dependencies]? We need to make sure the fake resembles the original. `FakeCounterComponent implements Partial<CounterComponent>` requires the class to implement a subset of `CounterComponent`. TypeScript enforces that the given properties and methods have the same types as in the original class.

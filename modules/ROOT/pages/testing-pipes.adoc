...
一旦翻译加载完成，``TranslatePipe``会保存它并通知Angular变更检测器。特别是，它通过调用 https://angular.io/api/core/ChangeDetectorRef#markForCheck[``ChangeDetectorRef``的markForCheck] 方法将相应的视图标记为已更改。
...
- https://angular.io/api/core/ChangeDetectorRef[Angular API参考：ChangeDetectorRef]
====
...
需要测试什么？我们需要检查 `{% raw %}{{ key | translate }}{% endraw %}` 是否计算为 `key1的翻译`。但是有两种需要测试的情况：
...
- https://angular.io/api/core/testing/fakeAsync[Angular API 参考：fakeAsync]
- https://angular.io/api/core/testing/tick[Angular API 参考：tick]

...
- Services store data. They hold an internal state. We can get or set the state.
  ...
  In the test, we check whether the state is changed correctly. Since the state should be held in private properties, we cannot access the state directly. We test the state change by calling public methods. We should not peek into the <<testing-principles#_black_box_vs_white_box_testing, black box>>.
...
For testing the Observable, we use the same pattern that we have used for <<testing-components#_testing_outputs, testing a Component Output>>:
...
This leads to a problem that is known from <<testing-components#_testing_outputs, testing Outputs>>: If the code under test is broken, the Observable never emits. The `next` callback with `expect` will not be called. Despite the defect, Jasmine thinks that all is fine.

...
在``FlickrService``的情况下，我们最好编写**单元测试**。Angular具有用于测试依赖于``HttpClient``的代码的强大辅助程序： https://angular.io/guide/http#testing-http-requests[`HttpClientTestingModule`]。
...
[NOTE]
====
- https://angular.io/guide/http[Angular指南：使用HTTP与后端服务通信]
- https://angular.io/api/common/http/HttpClient[Angular API参考：HttpClient]
- https://angular.io/guide/http#testing-http-requests[Angular指南：测试HTTP请求]
- https://angular.io/api/common/http/testing/HttpClientTestingModule[Angular API参考：HttpClientTestingModule]
====
...
在第二步中，我们使用  https://angular.io/api/common/http/testing/HttpTestingController[`HttpTestingController`]查找挂起的请求。这个类是 ``HttpClientTestingModule``的一部分。我们通过调用 `TestBed.inject(HttpTestingController)` 来获取实例。
...
[NOTE]
====
- https://angular.io/api/common/http/testing/HttpTestingController[Angular API参考文档：HttpTestingController]
- https://angular.io/api/common/http/testing/TestRequest[Angular API参考文档：TestRequest]
====
...
- https://angular.io/api/common/http/HttpErrorResponse[Angular API参考：HttpErrorResponse]
...
这使您能够以编程方式筛选所有请求并检查所有条件。候选请求是一个具有诸如``method``、`url`、`headers`、`body`、``params``等属性的 https://angular.io/api/common/http/HttpRequest[HttpRequest] 实例。
...
[NOTE]
====
- https://angular.io/api/common/http/HttpRequest[Angular API参考：HttpRequest]
- https://angular.io/api/common/http/testing/TestRequest[Angular API参考：TestRequest]
====
...
我们在测试依赖于``HttpClient``的服务时使用了``HttpClientTestingModule``。再举一个例子，还有 https://angular.io/api/router/testing/RouterTestingModule[`RouterTestingModule`] 用于测试依赖于``Router``和``Location``的服务。

...
We are going to render this Component. We need a standard ../testing-components/#configuring-the-testing-module[Component test setup] using the `TestBed`.
...
In the following specs, we need to access the input element. We use the standard approach: a `data-testid` attribute and the `findEl` ../testing-components/#testing-helpers[testing helper].
...
The template code already contains `data-testid` attributes. This is how we find and examine the elements in the test (see ../testing-components/#querying-the-dom-with-test-ids[Querying the DOM with test ids]).
...
The elements have have a `data-testid="page"` and `data-testid="pages"`, respectively. We use the ../testing-components/#testing-helpers[`expectText` testing helper] to check their text content.

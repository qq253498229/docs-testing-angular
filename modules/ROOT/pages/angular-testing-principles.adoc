...
- We can either provide an *original*, fully-functional implementation. In this case, we are writing an ../testing-principles/#integration-tests[integration test] that includes direct and indirect dependencies.
- Or we provide a *fake* implementation that does not have side effects. In this case, we are writing a ../testing-principles/#unit-tests[unit test] that tries to test the application part in _isolation_.
...
For example, some Angular developers use https://jestjs.io/[Jest] instead of Jasmine and Karma. Some use ../testing-components-with-spectator/#testing-components-with-spectator[Spectator] or the https://github.com/testing-library/angular-testing-library[Angular Testing Library] instead of using `TestBed` directly.
...
When running the tests in the ../example-applications/#the-counter-component[counter project], the browser output looks like this:
...
This guide recommends to activate one useful Jasmine configuration option: `failSpecWithNoExpectations` lets the test fail if it does not contain at least one expectation. (More on ../test-suites-with-jasmine/#expectations[expectations] later.) In almost all cases, specs without expectations stem from an error in the test code.

...
TIP: Original or fake

This is of immense importance for automated testing. In our test, we can decide how to deal with a dependency:

- We can either provide an *original*, fully-functional implementation. In this case, we are writing an <<testing-principles#_integration_tests, integration test>> that includes direct and indirect dependencies.
- Or we provide a *fake* implementation that does not have side effects. In this case, we are writing a <<testing-principles#_unit_tests, unit test>> that tries to test the application part in _isolation_.
...
[NOTE]
====
- https://angular.io/guide/dependency-injection[Angular guide: Dependency injection]
====
...
TIP: Balanced defaults

The Angular team already made decisions for you: https://jasmine.github.io/[Jasmine] as testing framework and https://karma-runner.github.io/[Karma] as test runner. Implementation and test code is bundled with https://webpack.js.org[Webpack]. Application parts are typically tested inside Angular’s https://angular.io/api/core/testing/TestBed[TestBed].
...
For example, some Angular developers use https://jestjs.io/[Jest] instead of Jasmine and Karma. Some use <<testing-components-with-spectator#_testing_components_with_spectator, Spectator>> or the https://github.com/testing-library/angular-testing-library[Angular Testing Library] instead of using `TestBed` directly.
...
TIP: Cast conventions into code
...
In our example application, the `CounterComponent` is located in https://github.com/9elements/angular-workshop/blob/main/src/app/components/counter/counter.component.ts[src/app/components/counter/counter.component.ts]. The corresponding test file sits in https://github.com/9elements/angular-workshop/blob/main/src/app/components/counter/counter.component.spec.ts[src/app/components/counter/counter.component.spec.ts]. This is an Angular convention, not a technical necessity, and we are going to stick to it.
...
When running the tests in the <<example-applications#_the_counter_component, counter project>>, the browser output looks like this:
...
3. The coverage reporter (npm package: `karma-coverage`) creates the test coverage report. See <<measuring-code-coverage#_measuring_code_coverage, measuring code coverage>>.
...
This guide recommends to activate one useful Jasmine configuration option: `failSpecWithNoExpectations` lets the test fail if it does not contain at least one expectation. (More on <<test-suites-with-jasmine#_expectations, expectations>> later.) In almost all cases, specs without expectations stem from an error in the test code.
...
[NOTE]
====
- https://karma-runner.github.io/6.4/config/configuration-file.html[Karma documentation: Configuration File]
- https://karma-runner.github.io/6.4/config/plugins.html[Karma documentation: Plugins]
- https://www.npmjs.com/search?q=keywords:karma-plugin[npm: List of Karma plugins]
- https://jasmine.github.io/api/edge/Configuration.html[Jasmine reference: Configuration options]
====

...
We have successfully tested the independent `CounterComponent` as well as the container `HomeComponent`. The next Component on our list is the https://github.com/9elements/angular-workshop/tree/main/src/app/components/service-counter[ServiceCounterComponent].
...
[NOTE]
====
- https://github.com/9elements/angular-workshop/blob/main/src/app/components/service-counter/service-counter.component.spec.ts[ServiceCounterComponent: integration test]
====
...
There are several practical approaches with pros and cons. We have discussed two main <<faking-dependencies#_faking_dependencies, requirements on fake dependencies>>:
...
TIP: Recommended faking approach
...
TIP: Pick public members

Luckily, we can use a TypeScript trick to fix this problem. Using https://www.typescriptlang.org/docs/handbook/utility-types.html#picktype-keys[Pick] and https://www.typescriptlang.org/docs/handbook/advanced-types.html#index-types[keyof], we create a derived type that only contains the public members:
...
`Pick` and other https://www.typescriptlang.org/docs/handbook/advanced-types.html#mapped-types[mapped types] help to bind the fake to the original type in a way that TypeScript can check the equivalence.
...
<<faking-dependencies#_faking_functions_with_jasmine_spies, Jasmine spies>> are suitable for this job. A first approach fills the fake with standalone spies:
...
`createSpyObj` accepts a https://www.typescriptlang.org/docs/handbook/generics.html[TypeScript type variable] to declare the type of the created object. We pass `CounterService` between angle brackets so TypeScript checks that the fake matches the original.
...
[NOTE]
====
- https://github.com/9elements/angular-workshop/tree/main/src/app/components/service-counter[ServiceCounterComponent: implementation and test code]
- https://angular.io/guide/dependency-injection-providers[Angular guide: Dependency providers]
====
...
[NOTE]
====
- https://github.com/9elements/angular-workshop/blob/main/src/app/components/service-counter/service-counter.component.spec.ts[ServiceCounterComponent: unit test]
====
...
2. Whichever approach you choose, make sure to meet the <<faking-dependencies#_faking_dependencies, basic requirements>>:
+
** 1. Equivalence of fake and original: The fake must have a type derived from the original.
** 2. Effective faking: the original stays untouched.
